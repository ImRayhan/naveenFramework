...
public class ElementUtil {
	...
	public By getBy(String locatorType, String locatorValue) {
		...
		default:
			...
			throw new FrameworkExeption("WRONG LOCATOR TYPE");
		}
	...
	@Step("cEntering value {1} to element {0}")
	...
	public Alert waitForJSAlertWithFluentWait(int timeOut, int intervalTime) {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(timeOut))
				.pollingEvery(Duration.ofSeconds(intervalTime))
				.withMessage("--time out is done...alert is not appeared....").ignoring(FrameworkExeption.class);
	...
	public WebElement retryingElement(By locator, int timeOut) {
		...
		if (element == null) {
			System.out.println("element is not found....tried for " + timeOut + " times " + " with the interval of "
					+ 500 + " milli seconds ");
			throw new FrameworkExeption("No Such Element");
		}
		ishighlight(element);
